...
## Переделка feel + графика (июль 2026)

- **Физика машин:** grip/lateral blend, brake/reverse, friction из `vehicles.json`, без Arcade drag
- **Камера:** lerp ~0.10–0.12, look-ahead по velocity, zoom tween
- **Трафик:** front-brake перед машиной впереди
- **Атмосфера:** blend day/night, radial vignette, street lights ночью
- **Анимации:** walk sheets NPC/пешеходов; тени/brake lights машин
...
- **Такси + смартфон:** job `taxi` (чистота, оценки, мойка); смена open/close для курьера/такси; **P** — смартфон (работа, навигация, еда, аренда); HUD предупреждает об аренде; медленнее hunger/sleep
...
```
(overhaul) Физика, камера, атмосфера, life-sim polish
